...
void krkw_init(struct kfd* kfd)
{
    ...
    if(!isarm64e() || kfd->info.env.vid <= 9) {
...
int krkw_helper_grab_free_pages(struct kfd* kfd)
{
    ...
    if((kfd->info.env.exploit_type == MEOW_EXPLOIT_LANDA && !isarm64e()) || (kfd->info.env.exploit_type == MEOW_EXPLOIT_LANDA && kfd->info.env.vid <= 9))
