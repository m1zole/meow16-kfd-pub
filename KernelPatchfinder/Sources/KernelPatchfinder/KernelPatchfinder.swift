...
open class KernelPatchfinder {
    ...
    public lazy var vn_kqfilter: UInt64? = {
        guard let str = cStrSect.addrOf("Invalid knote filter on a vnode!") else {
            return nil
        }
        
        guard var pc = textExec.findNextXref(to: str, optimization: .noBranches) else {
            return nil
        }
        
        let start: UInt64 = pc - 0x700

        guard var ret = textExec.addrOf([0xD503237F], startAt: start) else {
            return nil
        }
        return ret
    }()
    
    public lazy var perfmon_devices: UInt64? = {
        guard let str = cStrSect.addrOf("perfmon: %s: devfs_make_node_clone failed") else {
            return nil
        }
        
        guard var pc = textExec.addrOf([0xD503237F], startAt: textExec.findNextXref(to: str, optimization: .noBranches)) else {
            return nil
        }
        
        var i:UInt64 = 0
        while(i < 0x50) {
            var ret = AArch64Instr.Emulate.adrpAdd(adrp: textExec.instruction(at: pc + 0x40 + i) ?? 0, add: textExec.instruction(at: pc + i + 0x44) ?? 0, pc: pc + 0x40 + i)
            if(ret != nil) {
                return ret
            }
            i += 4
    ...
    public lazy var perfmon_dev_open: UInt64? = {
        guard let str = cStrSect.addrOf("perfmon: attempt to open unsupported source: 0x%x") else {
            return nil
        }
        
        guard var ret = textExec.addrOf([0xD503237F], startAt: (textExec.findNextXref(to: str, optimization: .noBranches) ?? 0) - 0x300) else {
            return nil
        }
        return ret
    }()
    
    public lazy var vm_pages: UInt64? = {
        guard let str = cStrSect.addrOf("vm pages array") else {
            return nil
        }
        
        guard var pc = textExec.addrOf([0xD503237F], startAt: (textExec.findNextXref(to: str, optimization: .noBranches) ?? 0) - 0x200) else {
            return nil
        }
        
        var i:UInt64 = 0
        while(i < 0x100) {
            var ret = AArch64Instr.Emulate.adrpLdr(adrp: textExec.instruction(at: pc + i) ?? 0, ldr: textExec.instruction(at: pc + i + 0x4) ?? 0, pc: pc + i)
            if(ret != nil) {
                return ret
            }
            i += 4
    ...
    public lazy var vm_page_array: (beginning: UInt64?, ending: UInt64?) = {
        //ending
        var beginning:UInt64? = nil
        var ending:UInt64?    = nil
        guard let str = cStrSect.addrOf("vm pages array") else {
        ...
        var i:UInt64 = 0
        while(i < 0x100) {
            ending = AArch64Instr.Emulate.adrpLdr(adrp: textExec.instruction(at: pc - i) ?? 0, ldr: textExec.instruction(at: pc - i + 0x4) ?? 0, pc: pc - i)
            if(ending != nil) {
                break
            }
            i += 4
        ...
        //beginning
        guard let str = cStrSect.addrOf("page_frame_init") else {
            return (beginning: beginning, ending: ending)
        }
        ...
        while(count < 5) {
            beginning = nil
            beginning = AArch64Instr.Emulate.adrpLdr(adrp: textExec.instruction(at: pc + j) ?? 0, ldr: textExec.instruction(at: pc + j + 0x4) ?? 0, pc: pc + j)
            if(beginning != nil) {
                count += 1
                if(count >= 2 && beginning != ending) {
                    break
                }
            }
            j += 4
